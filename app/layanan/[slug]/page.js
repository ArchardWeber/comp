      ...
      <section className={styles.hero} style={{ backgroundImage: `url(${service.images[0]})` }}>
        ...
        <div className={styles.heroContent}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: '16px' }}>
            <div style={{ width: '56px', height: '56px', background: 'rgba(227,30,36,0.85)', backdropFilter: 'blur(10px)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.2)' }}>
              <ServiceIcon slug={service.slug} size={28} color="white" />
            </div>
            <h1>{service.title}</h1>
          </div>
        </div>
              ...
              <div className={styles.highlights}>
                ...
                <ul>
                  {service.highlights.map((h, i) => (
                    <li key={i}>
                      <span className={styles.checkIcon}><CheckCircleIcon size={20} /></span>
                      {h}
                    </li>
                  ))}
                </ul>
